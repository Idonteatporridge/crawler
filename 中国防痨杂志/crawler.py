...
# 爬虫配置
BASE_URL = "https://www.zgflzz.cn"
ARCHIVE_URL = "https://www.zgflzz.cn/CN/article/showOldVolumn.do"
        ...
        for cell in row.find_all("td"):
            for link in cell.find_all("a"):
                ...
                if link_pattern.search(href):
                    issue_url = urljoin(BASE_URL, "/CN/" + href.lstrip(".."))
...
def extract_pdfs_from_volumn(vol_url):
    ...
    # 查找所有文章列表项
    article_list = soup.find("div", class_="articles")
    if not article_list:
        return pdfs
    ...
    for article_div in article_list.find_all("div", class_="noselectrow"):
        ...
        # 提取标题
        title_dd = content_div.find("dd")
        if title_dd:
            title_a = title_dd.find("a", class_="biaoti")
            if title_a:
                article_data["title"] = title_a.get_text(strip=True)
                # 提取DOI
                doi_href = title_a.get("href", "")
                if doi_href.startswith("https://www.zgflzz.cn/CN/10."):
                    article_data["doi"] = doi_href.split("/CN/")[1]
        
        # 提取作者
        authors_dd = title_dd.find_next("dd", class_="zuozhe")
        if authors_dd:
            article_data["authors"] = authors_dd.get_text(strip=True)
        
        # 提取PDF下载链接
        for a in content_div.find_all("a", class_="txt_zhaiyao1"):
            onclick_attr = a.get("onclick", "")
            if "lsdy1('PDF'" in onclick_attr:
                # 匹配PDF ID: lsdy1('PDF','19204',...) or lsdy1('PDF', '19204',...)
                pdf_id_match = re.search(r"lsdy1\('PDF',\s*'([^']+)',", onclick_attr)
                if pdf_id_match:
                    pdf_id = pdf_id_match.group(1)
                    article_data["pdf_url"] = f"{BASE_URL}/CN/article/downloadArticleFile.do?attachType=PDF&id={pdf_id}"
        ...
        # 从URL中提取年份和期号信息
        # 解析URL：https://www.zgflzz.cn/CN/volumn/volumn_1493.shtml
        vol_match = re.search(r"volumn_(\d+)\.shtml", vol_url)
        if vol_match:
            vol_num = vol_match.group(1)
            article_data["issue"] = vol_num
        
        # 提取年份信息 - 可以从PDF onclick事件中获取
        if "'2025'" in onclick_attr:
            article_data["year"] = "2025"
        # 或者尝试从页面其他位置获取年份
        else:
            year_meta = soup.find("meta", attrs={"name": "citation_publication_date"})
            if year_meta:
                year = year_meta.get("content", "")[:4]
                if year.isdigit():
