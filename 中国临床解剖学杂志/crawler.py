...
# 爬虫配置
BASE_URL = "https://www.chjcana.com"
ARCHIVE_URL = "https://www.chjcana.com/CN/article/showOldVolumn.do"
...
def extract_pdfs_from_volumn(vol_url):
    ...
    for article_div in article_list.find_all("div", class_="noselectrow"):
        ...
        # 提取DOI和PDF链接
        kmnjq_dd = content_div.find("dd", class_="kmnjq")
        if kmnjq_dd:
            doi_link = kmnjq_dd.find("a")
            if doi_link:
        ...
        # 提取年份和期号信息
        if kmnjq_dd:
            kmnjq_text = kmnjq_dd.get_text(strip=True)
