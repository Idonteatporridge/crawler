...
# 爬虫配置
BASE_URL = "https://zhsszz.xml-journal.net/"
ARCHIVE_URL = "https://zhsszz.xml-journal.net/archive_list.htm"
...
def get_all_volumn_links():
    ...
    links = []
    # 查找表格中的期号链接
    table = soup.find("table", class_="guokan-table")
    if not table:
        print("未找到过刊列表表格")
        return []
    
    # 查找所有期号链接
    for a in table.find_all("a", href=re.compile(r"/cn/article/\d+/\d+")):
        href = a["href"]
...
def extract_pdfs_from_volumn(vol_url):
    ...
    year = year_issue_match.group(1) if year_issue_match else "未知"
    issue = year_issue_match.group(2) if year_issue_match else "未知"
    
    # 提取卷号信息（从页面标题中）
    volume = "未知"
    title_element = soup.find("h2", class_="journal-text item-text-center")
    if title_element:
        volume_match = re.search(r"第(\d+)卷", title_element.get_text())
        if volume_match:
            volume = volume_match.group(1)
    
    # 查找所有文章列表项
    article_items = soup.find_all("div", class_="article-list")
    # print(f"找到 {len(article_items)} 个文章列表项")
    
    for item in article_items:
        # 查找PDF下载链接
        pdf_a = item.find("a", onclick=re.compile(r"downloadpdf\('([^']+)'\)"))
        if pdf_a:
            # 从onclick事件中提取ID
            onclick_match = re.search(r"downloadpdf\('([^']+)'\)", pdf_a["onclick"])
            if onclick_match:
                article_id = onclick_match.group(1)
                # 构建PDF下载链接
                pdf_url = f"{BASE_URL}/article/exportPdf?id={article_id}"
                ...
                # 提取文章标题
                title_div = item.find("div", class_="article-list-title")
                if title_div:
                    title = title_div.get_text(strip=True)
                
                # 提取作者信息
                author_div = item.find("div", class_="article-list-author")
                if author_div:
...
def main():
    ...
    if os.path.exists(CSV_FILE):
        with open(CSV_FILE, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                # 从文章信息中构建期号URL
                vol_url = f"{BASE_URL}cn/article/{row['year']}/{row['issue']}"
