...
# 爬虫配置
BASE_URL = "https://actamicro.ijournals.cn"
ARCHIVE_URL = "https://actamicro.ijournals.cn/actamicrocn/issue/browser"
...
def get_all_volumn_links():
    ...
    links = []
    # 查找所有期号链接
    for a in soup.find_all("a", href=re.compile(r"actamicrocn/article/issue/\d+_\d+_\d+")):
        href = a["href"]
        # 确保URL是完整的
        if not href.startswith("http"):
            # 如果href以"actamicrocn"开头，添加BASE_URL
            if href.startswith("actamicrocn"):
...
def extract_pdfs_from_volumn(vol_url):
    ...
    for article_des in soup.find_all("div", class_="article_des article_issue_fl"):
        ...
        # 从URL中提取年份和期号信息
        # URL格式如：https://actamicro.ijournals.cn/actamicrocn/article/issue/2026_66_1
        ...
        if pdf_url and not pdf_url.startswith("http"):
            if pdf_url.startswith("actamicrocn"):
