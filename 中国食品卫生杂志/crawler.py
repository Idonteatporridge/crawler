...
# 爬虫配置
BASE_URL = "https://www.zgspws.com"
ARCHIVE_URL = "https://www.zgspws.com/zgspwszz/issue/browser"
...
def get_all_volumn_links():
    ...
    links = []
    # 查找所有期号链接
    for a in soup.find_all("a", href=re.compile(r"zgspwszz/article/issue/\d+_\d+_\d+")):
        href = a["href"]
        # 构建完整URL
...
def extract_pdfs_from_volumn(vol_url):
    ...
    # 查找所有文章条目
    for li in article_list.find_all("li", class_="article_line"):
        # 提取标题
        title_a = li.find("div", class_="article_title").find("a") if li.find("div", class_="article_title") else None
        title = title_a.get_text(strip=True) if title_a else "无标题"
        
        # 提取PDF链接
        pdf_a = li.find("a", class_="btn_pdf")
        if not pdf_a:
            continue
        ...
        # 提取article_id
        art_id = ""
        file_no_input = li.find("input", class_="article_issue_fl", attrs={"name": "FileNO"})
        if file_no_input:
            art_id = file_no_input.get("value", "")
        ...
        # 从位置信息中提取
        position_p = li.find("p", class_="article_position")
        if position_p:
            position_text = position_p.get_text(strip=True)
            # 匹配格式："2025, 34(2):183-189."
            pos_match = re.search(r"(\d{4}),\s*(\d+)\((\d+)\):", position_text)
            if pos_match:
                year = pos_match.group(1)
                volume = pos_match.group(2)
                issue = pos_match.group(3)
        
        # 提取DOI
        doi = ""
        doi_a = li.find("span", string=re.compile(r"doi:"))
        if doi_a:
            doi_link = doi_a.find("a")
            if doi_link:
