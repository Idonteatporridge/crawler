...
# 药物不良反应杂志爬虫配置
BASE_URL = "https://www.cadrj.com"
ARCHIVE_URL = "https://www.cadrj.com/CN/archive_by_years?forwardJsp=simple"
CSV_FILE = "cadrj_all_pdfs.csv"
...
def extract_pdfs_from_volumn(vol_url):
    ...
    # 查找所有PDF下载链接
    for li in soup.find_all("li", id=re.compile(r"art\d+")):
        # 提取文章ID
        art_id_match = re.search(r"art(\d+)", li["id"])
        if not art_id_match:
            continue
        art_id = art_id_match.group(1)
        
        # 提取文章标题 - 根据实际HTML结构，class是j-title-1（数字1）而不是j-title-l1（字母l）
        title = "无标题"
        ...
        # 直接使用文章ID构建PDF链接，格式为https://www.cadrj.com/CN/PDF/218
        pdf_url = f"{BASE_URL}/CN/PDF/{art_id}"
        print(f"  构建的PDF URL: {pdf_url}")  # 调试信息
