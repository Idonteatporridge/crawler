...
# 爬虫配置
BASE_URL = "https://jsyx.magtechjournal.com"
ARCHIVE_URL = "https://jsyx.magtechjournal.com/CN/archive_by_issues"
...
def get_all_volumn_links():
    ...
    links = []
    # 查找所有期号链接
    for a in soup.find_all("a", href=re.compile(r"/CN/volumn/volumn_\d+\.shtml")):
        href = a["href"]
...
def extract_pdfs_from_volumn(vol_url):
    ...
    for li in soup.find_all("li", class_="noselectrow"):
        ...
        volumn_span = li.find("span", class_="j-volumn")
        if volumn_span:
            volumn_text = volumn_span.get_text(strip=True)
            # 匹配格式：2025, 49(10): 738-746.
            volumn_match = re.search(r"(\d{4}),\s*(\d+)\((\d+)\)", volumn_text)
            if volumn_match:
