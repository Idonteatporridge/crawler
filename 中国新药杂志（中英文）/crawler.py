...
# 爬虫配置
BASE_URL = "http://www.newdrug.cn"
ARCHIVE_URL = "http://www.newdrug.cn/CN/archive_by_years"
...
def get_all_volumn_links():
    ...
    # 正则表达式匹配期号链接，新格式：Y2025/V25/I12
    link_pattern = re.compile(r"Y\d+/V\d+/I\d+")
...
def extract_pdfs_from_volumn(vol_url):
    ...
    # 查找所有文章条目 - 直接查找所有li.noselectrow
    article_divs = soup.find_all("li", class_="noselectrow")
    if not article_divs:
        return pdfs
    ...
    for article_div in article_divs:
        ...
        # 查找文章内容div
        content_div = article_div.find("div", class_="article-l article-w")
        if not content_div:
            continue
        
        # 提取标题
        title_div = content_div.find("div", class_="j-title-1")
        if title_div:
        ...
        # 提取作者
        author_div = content_div.find("div", class_="j-author")
        if author_div:
            article_data["authors"] = author_div.get_text(strip=True)
        
        # 提取卷期信息和DOI
        # 尝试两种可能的类名拼写
        volume_doi_div = content_div.find("div", class_="j-voumn-doi")
        if not volume_doi_div:
            volume_doi_div = content_div.find("div", class_="j-volumn-doi")
        
        if volume_doi_div:
            # 提取卷期信息（尝试两种可能的类名拼写）
            volume_span = volume_doi_div.find("span", class_="j-voumn")
            if not volume_span:
                volume_span = volume_doi_div.find("span", class_="j-volumn")
                
            if volume_span:
                volume_text = volume_span.get_text(strip=True)
                # 匹配格式：中国新药杂志。2025，34(24)：2577-2584.
                year_match = re.search(r"(\d{4})", volume_text)
                if year_match:
                    article_data["year"] = year_match.group(1)
                
                issue_match = re.search(r"(\d+)\((\d+)\)", volume_text)
                if issue_match:
            ...
            # 提取DOI和PDF链接
            doi_a = volume_doi_div.find("a", class_="j-doi")
            if doi_a:
                doi_href = doi_a.get("href", "")
        ...
        # 从URL中提取年份和期号信息（如果之前没有获取到）
        # 解析URL：https://journal11.magtechjournal.com/cjcnn/CN/Y2025/V25/I12
        url_match = re.search(r"Y(\d+)/V(\d+)/I(\d+)", vol_url)
        if url_match:
            year = url_match.group(1)
            volume = url_match.group(2)
            issue_num = url_match.group(3)
            
            if article_data["year"] == "未知":
