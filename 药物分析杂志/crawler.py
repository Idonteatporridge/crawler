...
# 爬虫配置
BASE_URL = "https://ywfx.nifdc.org.cn"
ARCHIVE_URL = "https://ywfx.nifdc.org.cn/CN/archive_by_years"
...
def get_all_volumn_links():
    ...
    links = []
    # 查找所有期号链接
    # 新网站的期号链接在表格中，格式为 /CN/Y2025/V45/I8
...
def extract_pdfs_from_volumn(vol_url):
    ...
    for li in soup.find_all("li", class_="noselectrow"):
        ...
        # 提取DOI
        doi = ""
        doi_div = li.find("div", class_="j-volumn-doi")
        if doi_div:
            doi_a = doi_div.find("a", class_="j-doi")
        ...
        if pdf_a:
            ...
            art_id_match = re.search(r"lsdy1\('[^']+'\s*,'([^']+)'\)", onclick)
            if art_id_match:
        ...
        # 从URL中提取年份和期号信息
        # URL格式如：https://ywfx.nifdc.org.cn/CN/Y2025/V45/I8
        vol_match = re.search(r"/Y(\d+)/V\d+/I(\d+)", vol_url)
        if vol_match:
        ...
        if doi:
            # 从DOI构建PDF下载链接
            # DOI格式：https://doi.org/10.16155/j.0254-1793.2021.12.01
            # PDF URL格式：https://ywfx.nifdc.org.cn/CN/PDF/10.16155/j.0254-1793.2021.12.01
            doi_path = doi.replace("https://doi.org/", "")
            pdf_url = f"{BASE_URL}/CN/PDF/{doi_path}"
