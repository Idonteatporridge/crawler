...
# 爬虫配置
BASE_URL = "http://www.chinaoncology.cn/zgzl8/ch/reader"
ARCHIVE_URL = "http://www.chinaoncology.cn/zgzl8/ch/reader/issue_browser.aspx"
    ...
    if query_ui_table:
        ...
        for link in query_ui_table.find_all("a"):
            ...
            if href.startswith("issue_list.aspx?year_id="):
                # 确保正确构建URL，添加/reader路径http://www.chinaoncology.cn/zgzl8/ch/reader/issue_list.aspx?year_id=2026&quarter_id=1
                issue_url = f"http://www.chinaoncology.cn/zgzl8/ch/reader/{href}"
...
def extract_pdfs_from_volumn(vol_url):
    ...
    # 查找所有文章列表项
    # 1. 首先查找id为table24的表格，这是包含文章信息的主要表格
    table24 = soup.find("table", id="table24")
    if table24:
        # 查找表格中的所有行
        rows = table24.find_all("tr")
        ...
        while i < len(rows):
            ...
            if title_link:
                ...
                # 提取file_no作为article_id（支持带A开头的格式）
                file_no_match = re.search(r"file_no=([A-Za-z0-9]+)", title_link.get("href", ""))
                if file_no_match:
                    article_data["article_id"] = file_no_match.group(1)
                
                # 提取PDF下载链接 - 查找包含create_pdf的链接
                # 遍历后续行，寻找PDF链接
                j = i
                while j < len(rows):
                    pdf_link = rows[j].find("a", href=re.compile(r"create_pdf.aspx"))
                    if pdf_link:
                        ...
                        if pdf_href.startswith("http"):
                            ...
                            # 如果是相对路径，使用BASE_URL构建
                            article_data["pdf_url"] = f"http://www.chinaoncology.cn/zgzl8/ch/reader/{pdf_href}"
                        break
                    j += 1
    ...
    # 2. 如果没有找到PDF链接，尝试另一种方法：查找所有PDF链接
    if not pdfs:
        # 查找所有包含create_pdf的链接
        all_pdf_links = soup.find_all("a", href=re.compile(r"create_pdf.aspx"))
        
        for pdf_link in all_pdf_links:
            pdf_href = pdf_link.get("href", "")
            
            # 提取file_no（支持带A开头的格式）
            file_no_match = re.search(r"file_no=([A-Za-z0-9]+)", pdf_href)
            if file_no_match:
                ...
                if pdf_href.startswith("http"):
                    ...
                    pdf_url = f"http://www.chinaoncology.cn/zgzl8/ch/reader/{pdf_href}"
                ...
                # 尝试找到对应的文章标题
                title_link = soup.find("a", href=re.compile(r"view_abstract.aspx\?file_no=" + re.escape(file_no)))
                if title_link:
                    article_data["title"] = title_link.get_text(strip=True)
